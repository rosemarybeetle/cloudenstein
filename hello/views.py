...
def autho (request):
	...
	pw_st=auths[0].pw
	getMentioniser()
	ment_text="<script type='text/javascript'>var mention_text;var datetime;function fill_foam(){ var wordy = document.getElementById('entry').value;document.getElementById('foam').innerHTML+='inside fill_framer - passed argument = '+wordy; }function getDate(){ var currentdate = new Date(); datetime = 'Last Sync: ' + currentdate.getDate() + '/'+ (currentdate.getMonth()+1)  + '/' + currentdate.getFullYear() + ' @ '+ currentdate.getHours() + ':'  + currentdate.getMinutes() + ':' + currentdate.getSeconds();}function mentions() {	$.getJSON( 'http://cloudenstein.rosemarybeetle.org/recent_mentions', function(mens) {try {var cont=mens.countif (cont>0){mention_text=''; // clear mention_textfor ( var i = 0; i < cont; i++ ) {getDate()mention_text=datetime+': - New mention number '+String(i)+ 'with id:'+mens.mentions[i].status_id+' from user: @'+mens.mentions[i].screen_name+'('+mens.mentions[i].name+').<br />document.getElementById('foam').innerHTML+=mention_text;console.log('username '+String(i)+ '= '+mens.mentions[0].screen_name);console.log('id '+String(i)+ '= '+mens.mentions[0].status_id);console.log('name '+String(i)+ '= '+mens.mentions[0].name);}} else {getDate()mention_text=''; // clear mention_textdocument.getElementById('foam').innerHTML+=datetime+': '+mens.message+'<br />';console.log('no new mentions')}//speakTest(textu);}catch (err){console.log('error on loading JSON = '+err)}});}window.periody={{period}}*1000;window.toggler=1;function harvest (){document.getElementById('framer').src = 'http://cloudenstein.rosemarybeetle.org/recent_mentions';console.log('inside harvest')}=var vari; // global variablefunction toggle_harvesting (){if (window.toggler==1){console.log('inside toggle_harvesting() toggler=1')vari = setInterval(function(){ mentions()},window.periody);//vari = setInterval(function(){ mentions()},window.periody);window.toggler=0;}else if (window.toggler==0){clearInterval(vari);console.log('inside toggle_harvesting() toggler=0')window.toggler=1;}}</script><h1>Mentionising...</h1><input id='entry' type='text' value='default'> <button onClick='fill_foam()'>test</button><button onClick='toggle_harvesting()'>Go</button><br /><div id='foam'>toad<br /></div><div><p>hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world.. </p><p>  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world..  hello world hello world.. </p></div><iframe id='framer' src='' style='border: none;' width=100%></iframe><p></p>"
	try:
...
def getMentioniser():
	...
	ment_text+="function fill_foam()"
	ment_text+="{ var wordy = document.getElementById('entry').value;"
	ment_text+="document.getElementById('foam').innerHTML+='inside fill_framer - passed argument = '+wordy; "
	ment_text+="}"
	ment_text+="function getDate()"
	ment_text+="{ var currentdate = new Date(); "
	ment_text+="datetime = 'Last Sync: ' + currentdate.getDate() + '/'"
	ment_text+="+ (currentdate.getMonth()+1)  + '/' "
	ment_text+="+ currentdate.getFullYear() + ' @ '"
	ment_text+="+ currentdate.getHours() + ':'  "
	ment_text+="+ currentdate.getMinutes() + ':' "
	ment_text+="+ currentdate.getSeconds();"
	ment_text+="}"
	ment_text+="function mentions() "
	ment_text+="{	$.getJSON( 'http://cloudenstein.rosemarybeetle.org/recent_mentions', function(mens) {"
	ment_text+="try {"
	ment_text+="var cont=mens.count"
	ment_text+="if (cont>0){"
	ment_text+="mention_text=''; // clear mention_text"
	ment_text+="for ( var i = 0; i < cont; i++ ) {"
	ment_text+="getDate()"
	ment_text+="mention_text=datetime+': - New mention number '+String(i)+ 'with id:'+mens.mentions[i].status_id+' from user: @'+mens.mentions[i].screen_name+'('+mens.mentions[i].name+').<br />";
	ment_text+="document.getElementById('foam').innerHTML+=mention_text;"
	ment_text+="console.log('username '+String(i)+ '= '+mens.mentions[0].screen_name);"
	ment_text+="console.log('id '+String(i)+ '= '+mens.mentions[0].status_id);"
	ment_text+="console.log('name '+String(i)+ '= '+mens.mentions[0].name);"
	ment_text+="}"
	ment_text+="} else {"
	ment_text+="getDate()"
	ment_text+="mention_text=''; // clear mention_text"
	ment_text+="document.getElementById('foam').innerHTML+=datetime+': '+mens.message+'<br />';"
	ment_text+="console.log('no new mentions')"
	ment_text+="}"
	ment_text+="//speakTest(textu);"
	ment_text+="}"
	ment_text+="catch (err){"
	ment_text+="console.log('error on loading JSON = '+err)"
	ment_text+="}"
	ment_text+="});"
	ment_text+="}"
	ment_text+="window.periody={{period}}*1000;"
	ment_text+="window.toggler=1;"
	ment_text+="function harvest (){"
	ment_text+="document.getElementById('framer').src = 'http://cloudenstein.rosemarybeetle.org/recent_mentions';"
	ment_text+="console.log('inside harvest')"
	ment_text+="}"
	ment_text+="=var vari; // global variable"
	ment_text+="function toggle_harvesting (){"
	ment_text+="if (window.toggler==1){"
	ment_text+="console.log('inside toggle_harvesting() toggler=1')"
	ment_text+="vari = setInterval(function(){ mentions()},window.periody);"
	ment_text+="//vari = setInterval(function(){ mentions()},window.periody);"
	ment_text+="window.toggler=0;"
	ment_text+="}"
	ment_text+="else if (window.toggler==0)"
	ment_text+="{"
	ment_text+="clearInterval(vari);"
	ment_text+="console.log('inside toggle_harvesting() toggler=0')"
	ment_text+="window.toggler=1;"
	ment_text+="}"
	ment_text+="}"
	ment_text+="</script>"
