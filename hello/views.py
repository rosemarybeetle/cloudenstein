...
# ---------------seach_tweets is from older Tweetenstein - to be modded ------------------
# ----------------------------------------------------------------------------------------
def search_tweets (term,count) : # params: term= 'what to search for' type = 'how to search' Count = 'number of tweets' (max 100)
    ...
    try:
        ...
        response = HttpResponse(auth_response.text) # - uncomment to check the text is returning as expected
		return response # - uncomment to check the text is returning as expected
